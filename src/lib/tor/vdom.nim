...
method render*(ti: TorInfo): VNode {.base.} =
        ...
        tbody():
          tr():
            ...
            td():
              strong(style={display: "flex"}):
                tdiv():
                  text if ti.status.isTor: "Online" else: "Offline"
          ...
          tr():
            ...
            td():
              strong():
                tdiv():
                  text if ti.bridge.isObfs4: "On" else: "Off"
          tr():
            td(): text "Meek-Azure"
            td():
              strong():
                tdiv():
                  text if ti.bridge.isMeekazure: "On" else: "Off"
          tr():
            td(): text "Snowflake"
            td():
              strong():
                tdiv():
                  text if ti.bridge.isSnowflake: "On" else: "Off"

# method jsRender*(ti: TorInfo): VNode {.base.} =
#   buildHtml(tdiv(class="columns")):
#     tdiv(class="card card-padding card-tor"):
#       tdiv(class="card-header"):
#         text "Tor"
